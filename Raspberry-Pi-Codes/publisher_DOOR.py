...
and publish the data to an MQTT broker. The data collected includes the distance measurement
and door condition (Open/Closed).
...
2. Update the `ser = serial.Serial('COM3', 9600)` line with the Arduino's port.
...
Note: Ensure you have an MQTT broker running and accessible at the specified address and port.
"""
...
def Data_collection():
    ...
    condition = "Open"  # Default value

    try:
        distance, condition = data.split(",")
    except ValueError:
        print(f"Unexpected data format: {data}. Expected format: 'distance,condition'")

    Final_Data_collection= [distance, condition]

    return Final_Data_collection
    
def create_publisher_topics():
    publisher_topic_labels = ["distance", "door"]
